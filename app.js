...
async function searchMovie() {
  ...
  let movies = await fetch(`https://www.omdbapi.com/?apikey=aca2f055&s=${movieTitle}&type=movie&y=?`);
  let moviesData = await movies.json();
  console.log(moviesData)
  movieLists.innerHTML = moviesData.Search.map((element) => postHTML(element)).join("");
}

async function renderMovies(filter) {
  const movies = await fetch(`https://www.omdbapi.com/?apikey=aca2f055&s=${movieTitle}&type=movie&y=?`);
